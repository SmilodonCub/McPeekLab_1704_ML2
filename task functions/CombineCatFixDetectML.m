function [ target_fix_duration, left_fix_test] = CombineCatFixDetectML( eyedata, target_location , target_window_radius)
...
%find the distance between the x,y eye position data and the fixation's
%center location
fix_location = [ 0 0 ];
target_distance = sqrt( ( fix_location( 1 ) - eyedata( :,1 ) ).^2 + ...
    ( fix_location( 2 ) - eyedata( :,2 ) ).^2 );
%outCueWindow true = all the eyedata values that lie outside the fixation
%position window
offFixWindow = gt( target_distance, target_window_radius );
%total of true values for inCueWindow = time spent outside of cue window
%during idleTime (msec)
left_fix_sum = sum( offFixWindow );
%left_cue_test == true is left_cue_sum > 0 | false if left_cue_sum = 0
left_fix_test = any( left_fix_sum );
